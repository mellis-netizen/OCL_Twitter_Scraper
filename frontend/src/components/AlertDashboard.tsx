...
export default function AlertDashboard() {
  ...
  const { alerts, realtimeAlerts, isLoading, refetch } = useAlerts(filter);

  const handleFilterChange = (key: keyof AlertFilter, value: any) => {
        ...
        <div className="bg-primary-900 bg-opacity-30 border border-primary-700 rounded-lg p-4">
          ...
          <div className="space-y-2">
            {realtimeAlerts.slice(0, 5).map((alert, idx) => (
              <div
                key={idx}
                className="bg-dark-800 rounded p-3 border border-primary-600 animate-pulse-slow"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                handleFilterChange('urgency_level', e.target.value || undefined)
              }
              className="w-full px-3 py-2 bg-dark-700 border border-dark-600 rounded text-white text-sm"
            >
